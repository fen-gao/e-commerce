...
const DrawerQuantity = ({
  ...
  item: Products;
  handleQuantityChange: (key: "add" | "reduce", quantity?: number) => void;
}) => {
  return (
...
const DrawerSubtotal = ({ item }: { item: Products }) => {
  return (
...
const DrawerItem = ({ item }: { item: Products }) => {
  ...
  return (
    <div className="grid grid-cols-4 justify-start items-center w-full border-b-[1px] pb-2 h-[90px] gap-10 mt-2">
      ...
      <DrawerQuantity handleQuantityChange={handleQuantityChange} item={item} />

      <DrawerSubtotal item={item} />

      <ClearItem handleQuantityChange={handleQuantityChange} item={item} />
    </div>
  );
};

export const DrawerItemList = ({ item }: { item: Products[] }) => {
  return (
    <>
      {item.map((item) => (
